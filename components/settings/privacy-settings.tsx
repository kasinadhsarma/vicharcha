...
import { DigiLockerVerifyButton } from "@/components/auth/digilocker-verify-button";
...
export function PrivacySettings() {
  ...
  const fetchAdultContentPreference = async () => {
    ...
    try {
      const response = await fetch(`/api/settings/adult-content?userId=${user?.phoneNumber || user.id}`);
      const data = await response.json();
      if (response.ok) {
  ...
  const handleAdultContentToggle = async () => {
    ...
    try {
      const response = await fetch('/api/settings/adult-content', {
        ...
        body: JSON.stringify({
          userId: user?.phoneNumber || user.id,
